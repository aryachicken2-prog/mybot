    ...
    async def admin_callback_handler(event):
        ...
        # Handle cost type selection during new event creation
            ...
            buttons = [
                ...
                [Button.inline("📖 ویرایش راهنمای ربات", b"admin_edit_user_help")],
                [Button.inline("🏠 بازگشت", b"main_menu")],
        ...
        elif data == "toggle_console_logs":
            # Allow owners and admins to toggle console log language
            if not utils_is_admin(DB_NAME, user_id):
                await event.answer("❌ دسترسی ندارید!", alert=True)
                return
            try:
                cur = get_setting('console_logs_english', '1')
                new = '0' if cur == '1' else '1'
                set_setting('console_logs_english', new)
                en_on = new == '1'
                label = "✅ English logs enabled" if en_on else "❌ English logs disabled"
            except Exception:
                # fallback
                label = "⚠️ Toggle failed"
            # inform admin and refresh menu
            try:
...
💰 هزینه: {cost_display}
        ...
        elif data == "admin_actions_history":
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
            c.execute("SELECT id, admin_id, action, target_table, target_id, note, created_at FROM admin_actions ORDER BY id DESC LIMIT 50")
            rows = c.fetchall()
            conn.close()
            if not rows:
                await event.edit("📭 هیچ عملیاتی ثبت نشده است.", buttons=[[Button.inline("🏠 منوی اصلی", b"main_menu")]])
                return
            lines = []
            for aid, admin_id2, action, ttable, tid, note, created in rows:
                short = note[:60].replace('\n',' ') if note else ''
                lines.append(f"#{aid} — {action} on {ttable}#{tid} by {admin_id2} at {created}\n{short}")
            text = "📜 تاریخچه آخرین عملیات ادمین:\n\n" + "\n\n".join(lines)
            # show as plain message with a back button
            await event.edit(text, buttons=[[Button.inline("🏠 منوی اصلی", b"main_menu")]])
            ...
            conn.close()
            if not rows:
                await event.edit("📭 هیچ درخواستی وجود ندارد.", buttons=[[Button.inline("🏠 منوی اصلی", b"main_menu")]])
                return
            buttons = []
            ...
            buttons = [
                ...
                [Button.inline("❌ کاربران رد شده", b"broadcast_rejected"), Button.inline("🏠 بازگشت به منو", b"main_menu")]
            ]
        ...
        # ---- New admin handlers: ideas / collaborations / donations ----
            ...
            conn.close()
            if not rows:
                await event.edit("📭 هیچ ایده‌ای وجود ندارد.", buttons=[[Button.inline("🏠 منوی اصلی", b"main_menu")]])
                return
            buttons = []
            ...
            conn.close()
            if not rows:
                await event.edit("📭 هیچ حمایتی ثبت نشده است.", buttons=[[Button.inline("🏠 منوی اصلی", b"main_menu")]])
                return
            buttons = []
            ...
            conn.close()
            if not tickets:
                await event.edit("📭 هیچ تیکت بازی وجود ندارد.", buttons=[[Button.inline("🏠 منو", b"main_menu")]])
                return
            buttons = []
